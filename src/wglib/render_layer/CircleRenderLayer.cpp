...
std::optional<wgpu::RenderPipeline> CircleRenderLayer::m_render_pipeline{std::nullopt};

CircleRenderLayer::CircleRenderLayer(glm::vec2 origin, float radius, glm::vec3 color, uint32_t resolution)
    : m_origin(origin), m_radius(radius), m_color(color), m_resolution(resolution)
{
    assert(m_resolution >= 3 && "Resolution must be at least 3 to form a circle");
    calculateVertices();
}

auto CircleRenderLayer::Render(wgpu::RenderPassEncoder &renderPassEncoder) const -> void
{
    renderPassEncoder.SetPipeline(m_render_pipeline.value());
    renderPassEncoder.SetVertexBuffer(0, m_vertex_buffer);
    renderPassEncoder.SetIndexBuffer(m_index_buffer, wgpu::IndexFormat::Uint32, 0, m_indices.size() * sizeof(uint32_t));
    renderPassEncoder.DrawIndexed(static_cast<uint32_t>(m_indices.size()));
}

auto CircleRenderLayer::initRenderPipeline(const wgpu::Device &device, wgpu::TextureFormat format,
                                           const wgpu::BindGroupLayout &bindGroupLayout) -> void
{
    ...
    wgpu::PipelineLayoutDescriptor layoutDesc{.bindGroupLayoutCount = 1, .bindGroupLayouts = &bindGroupLayout};
    ...
    m_render_pipeline = std::make_optional(device.CreateRenderPipeline(&descriptor));
    util::log("Created render pipeline");
}

auto CircleRenderLayer::InitRes(const wgpu::Device &device, wgpu::TextureFormat format,
                                const wgpu::BindGroupLayout &bindGroupLayout) -> void
{
    if (m_isInitialized)
        return;

    const wgpu::BufferDescriptor vertexBufferDesc{.usage = wgpu::BufferUsage::Vertex | wgpu::BufferUsage::CopyDst,
                                                  .size = sizeof(Vertex) * m_vertices.size(),
                                                  .mappedAtCreation = true};

    m_vertex_buffer = device.CreateBuffer(&vertexBufferDesc);
    if (m_vertex_buffer.WriteMappedRange(0, m_vertices.data(), sizeof(Vertex) * m_vertices.size()) !=
        wgpu::Status::Success)
    {
    ...
    const wgpu::BufferDescriptor indexBufferDesc{.usage = wgpu::BufferUsage::Index | wgpu::BufferUsage::CopyDst,
                                                 .size = sizeof(uint32_t) * m_indices.size(),
                                                 .mappedAtCreation = true};
    m_index_buffer = device.CreateBuffer(&indexBufferDesc);
    if (m_index_buffer.WriteMappedRange(0, m_indices.data(), sizeof(uint32_t) * m_indices.size()) !=
        wgpu::Status::Success)
    {
...
auto CircleRenderLayer::UpdateRes(const wgpu::Device &device) const -> void
{
        ...
        m_index_buffer.GetSize() < sizeof(uint32_t) * m_indices.size())
    {
        ...
        util::log("Realloc vertex and index buffer to {}b from {}b", m_vertices.size(),
                  m_vertex_buffer.GetSize() / sizeof(Vertex));
        m_vertex_buffer.Destroy();
        const wgpu::BufferDescriptor descriptor{.usage = wgpu::BufferUsage::Vertex | wgpu::BufferUsage::CopyDst,
                                                .size = sizeof(Vertex) * m_vertices.size(),
    ...
    if (m_vertex_buffer_dirty)
    {
        ...
        queue.WriteBuffer(m_vertex_buffer, 0, m_vertices.data(), sizeof(Vertex) * m_vertices.size());
        m_vertex_buffer_dirty = false;
    }
    if (m_index_buffer_dirty)
    {

        queue.WriteBuffer(m_index_buffer, 0, m_indices.data(), sizeof(uint32_t) * m_indices.size());
...
auto CircleRenderLayer::calculateVertices() -> void
{
    ...
    {
        float angle = (i / static_cast<float>(m_resolution)) * 2.0f * glm::pi<float>();
...
auto CircleRenderLayer::getOrigin() const -> glm::vec2
{
    return m_origin;
}
...
auto CircleRenderLayer::getRadius() const -> float
{
    return m_radius;
}
...
auto CircleRenderLayer::getColor() const -> glm::vec3
{
    return m_color;
}
