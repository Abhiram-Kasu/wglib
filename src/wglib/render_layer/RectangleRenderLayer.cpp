...
{
std::optional<wgpu::RenderPipeline> RectangleRenderLayer::m_render_pipeline{std::nullopt};

RectangleRenderLayer::RectangleRenderLayer(const glm::vec2 position, const glm::vec2 size, const glm::vec3 color)
...
auto RectangleRenderLayer::InitRes(const wgpu::Device &device, const wgpu::TextureFormat format,
                                   const wgpu::BindGroupLayout &bindGroupLayout) -> void
{
    ...
    m_vertex_buffer.WriteMappedRange(0, m_vertices, sizeof(Vertex) * 6);
...
auto RectangleRenderLayer::initRenderPipeline(const wgpu::Device &device, wgpu::TextureFormat format,
                                              const wgpu::BindGroupLayout &bindGroupLayout) -> void
{
...
auto RectangleRenderLayer::Render(wgpu::RenderPassEncoder &renderPassEncoder) const -> void
{
...
auto RectangleRenderLayer::getSize() const -> glm::vec2
{
    return m_size;
}
...
auto RectangleRenderLayer::getColor() const -> glm::vec3
{
    return m_color;
}
