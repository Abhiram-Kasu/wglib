...
Engine::Engine(glm::vec2 size, std::string_view title) : m_window_size(size)
{
    ...
    const wgpu::InstanceDescriptor instanceDesc{.requiredFeatureCount = 1, .requiredFeatures = &K_TIMED_WAIT_ANY};
    m_instance = wgpu::CreateInstance(&instanceDesc);

    const auto f1 =
        m_instance.RequestAdapter(nullptr, wgpu::CallbackMode::WaitAnyOnly,
    ...
    if (const auto res = m_instance.WaitAny(f1, UINT64_MAX); res != wgpu::WaitStatus::Success)
    {
    ...
    desc.SetDeviceLostCallback(wgpu::CallbackMode::AllowSpontaneous,
                               [](const wgpu::Device &, wgpu::DeviceLostReason reason, wgpu::StringView stringView) {
    ...
    desc.SetUncapturedErrorCallback([](const wgpu::Device &, wgpu::ErrorType errorType, wgpu::StringView message) {
        util::log("Error: {} - message: {}", static_cast<uint32_t>(errorType), message.data);
    });

    wgpu::Future f2 =
        m_adapter.RequestDevice(&desc, wgpu::CallbackMode::WaitAnyOnly,
    ...
    if (const auto res = m_instance.WaitAny(f2, UINT64_MAX); res != wgpu::WaitStatus::Success)
    {
        ;
    ...
    // Create Window
    this->m_window_manager = std::make_unique<WindowManager>(size.x, size.y, title, m_instance, m_device, m_adapter);

    // Pass to Renderer
    //
    this->m_input_manager = std::make_unique<InputManager>(m_window_manager->window());

    this->m_renderer = std::make_unique<Renderer>(m_instance, m_adapter, m_device, m_window_manager->format(), size);
...
auto Engine::Input() -> const InputManager &
{
