...
{
class ParticleSimulationLayer : public ComputeLayer<std::optional<wgpu::Texture>>
{
  ...
  private:
    ...
    static auto genParticlesInSquareFormation(uint32_t numBalls, glm::vec2 size, glm::vec2 start, uint32_t numPerRow,
                                              float ballRadius, glm::vec4 color) -> std::vector<Particle>;
  ...
  public:
    ParticleSimulationLayer(uint32_t numBalls, glm::vec2 size, uint32_t circleRadius, glm::vec4 ballColor,
                            glm::vec2 startLocation, uint32_t numPerRow, float dt, float gravity, float damping,
                            float forceAmp, float decayLength);
