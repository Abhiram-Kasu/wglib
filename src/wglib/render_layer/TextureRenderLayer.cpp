...
TextureRenderLayer::TextureRenderLayer(wgpu::Texture texture, float width, float height)
    : m_texture(std::move(texture)), m_width(width), m_height(height), m_isDirty(true)
{
}

TextureRenderLayer::TextureRenderLayer(float width, float height)
    : m_texture(std::nullopt), m_width(width), m_height(height), m_isDirty(true)
{
}
...
void TextureRenderLayer::Render(wgpu::RenderPassEncoder &renderPassEncoder) const
{
...
void TextureRenderLayer::InitRes(const wgpu::Device &device, wgpu::TextureFormat format, const wgpu::BindGroupLayout &)
{
    // Vertex buffer
    ...
    device.GetQueue().WriteBuffer(m_vertexBuffer, 0, vertices.data(), bufferDesc.size);

    // Shader
    const auto shaderModule = util::createShaderModuleFromFile(util::shaderPath("texture.wgsl"), device);
