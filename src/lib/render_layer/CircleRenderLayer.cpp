...
std::optional<wgpu::RenderPipeline> CircleRenderLayer::renderPipeline{std::nullopt};

CircleRenderLayer::CircleRenderLayer(glm::vec2 origin, float radius, glm::vec3 color, uint32_t resolution = 50)
...
auto CircleRenderLayer::Render(wgpu::RenderPassEncoder &renderPassEncoder) const -> void
{
    ...
    renderPassEncoder.SetIndexBuffer(m_index_buffer, wgpu::IndexFormat::Uint32);
...
auto CircleRenderLayer::UpdateRes(wgpu::CommandEncoder &commandEncoder) const -> void
{
    if (m_vertex_buffer_dirty)
    {
        if (m_vertex_buffer.GetSize() < sizeof(Vertex) * m_vertices.size())
        {
            // TODO handle buffer recreation if needed with different size
            // Would also need to realloc index buffer since the this branch is
            // only if the user changed the resolution of the circle to larger value
            std::println("Vertex buffer size insufficient");
            exit(0);
        }
        commandEncoder.WriteBuffer(m_vertex_buffer, 0, reinterpret_cast<const uint8_t *>(m_vertices.data()),
