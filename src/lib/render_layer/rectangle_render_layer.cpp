...
namespace wglib::render_layers
{
    ...
    auto RectangleRenderLayer::Render(wgpu::RenderPassEncoder& renderPassEncoder) const
        -> void
    {
    ...
    auto RectangleRenderLayer::UpdateRes(wgpu::CommandEncoder& commandEncoder) const -> void
    {
        if (m_vertex_buffer_dirty)
        {
            commandEncoder.WriteBuffer(m_vertex_buffer, 0, reinterpret_cast<const uint8_t*>(m_vertices),
                                       sizeof(Vertex) * 6);
        }
    }
