...
class ComputeEngine
{
  ...
  public:
    template <typename TResult> struct ComputeLayerHandle
    {
      ...
      private:
        ...
        ComputeLayerHandle(std::shared_ptr<ComputeLayer<TResult>> layer) : m_compute_layer(std::move(layer))
        {
        }
    };
  ...
  public:
    ...
    auto InitComputeLayer(Args &&...args) -> ComputeLayerHandle<typename LayerType::ResultType>
    {
    ...
    auto PushComputeLayer(ComputeLayerHandle<TResult> &handle, CB &&onComplete) -> void
    {

        auto completion = [layer = handle.m_compute_layer, cb = std::move(onComplete)]() mutable {
            auto result = layer->getResult();
            cb(std::move(result));
        };

        m_computeQueue.push(ComputeTask{handle.m_compute_layer, std::move(completion)});
    }
};
