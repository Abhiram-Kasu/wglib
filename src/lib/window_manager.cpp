...
WindowManager::WindowManager(uint32_t width, uint32_t height,
                             std::string_view title, wgpu::Instance &instance, wgpu::Device &device,
...
auto WindowManager::configureSurface(wgpu::Device &device, wgpu::Adapter &adapter) -> void
{
    ...
    wgpu::SurfaceConfiguration config{
        .device = device, .format = m_format, .width = m_width, .height = m_height
    };
    m_surface.Configure(&config);
}

auto WindowManager::width() const -> uint32_t
{
    return m_width;
}

auto WindowManager::height() const -> uint32_t
{
    return m_height;
}
